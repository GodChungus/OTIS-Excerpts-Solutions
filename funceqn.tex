...
\begin{proof}
    ...
    f(a-b) = f(a) - f(b)
    \]
    Again, we can set $a=x_0 + y_0$ and $b=y_0$ for some nonnegative $x_0$, $y_0$ to get:
    \[
    f(x_0+y_0-y_0) = f(x_0+y_0) - f(y_0) \implies f(x_0 + y_0) = f(x_0) + f(y_0)
    \]
    for nonnegative $x, y$. However, since $f$ is odd, we get that $f$ is additive on the negatives too.
