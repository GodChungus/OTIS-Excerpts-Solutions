...
\begin{proof}
  ...
  \]
  Which gives us that $f(x) = x$ or $\frac 1x$. However, as $f$ could be a piecewise function, we prove the nonexistence of one.
  \newline \newline
  ...
  \]
  If we take $f(ab) = ab$, we get $b=1$ and the case where $f(ab) = \frac{1}{ab}$ gives $a=1$, contradicting that $a$ and $b$ are distinct.
